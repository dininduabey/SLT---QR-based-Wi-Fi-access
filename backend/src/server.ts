        ...
        if (ev.policies?.dataLimitMb) {
            ...
            if (!dt || dt.status === 'exhausted') {
                ...
                if (!dt.macAddress && mac !== 'unknown') {
                    await DataTokenModel.updateOne({ tokenId: dt.tokenId }, { $set: { macAddress: mac.toLowerCase() } });
...
app.post('/internal/radius-check', async (req: Request, res: Response) => {
    try {
        ...
        if (rawMac) {
            ...
            if (!mac) return res.json({ accept: false, reason: 'no mac' });

            const token = await DataTokenModel.findOne(
                { macAddress: mac, status: 'active' },
                null, { sort: { createdAt: -1 } }
            );
            if (!token) return res.json({ accept: false, reason: 'no active token' });

            const quotaBytes = Math.max(0,
            ...
            const token = await DataTokenModel.findOne({ tokenId, status: 'active' });
            if (!token) return res.json({ accept: false, reason: 'no active token' });
...
app.get('/portal/active', async (req: Request, res: Response) => {
    try {
        const ev = await EventModel.findOne({ status: 'active' }, {}, { sort: { createdAt: -1 } });
        if (!ev) return res.redirect('http://124.43.216.136:45080/');
